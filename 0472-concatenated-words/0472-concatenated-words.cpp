    ...
    bool isConcatenated(string word, unordered_set<string>& st) {
        ...
        for (int i = 0; i < l; i++) {
            ...
            if (st.find(prefix) != st.end() && 
                (isConcatenated(suffix, st) || st.find(suffix) != st.end())) {
...
// class Solution {
// public:
//     unordered_map<string, bool> memo; // memoization to avoid recomputation
...
//             // Case 1: prefix is a word AND suffix is a word
//             if (dict.find(prefix) != dict.end() && dict.find(suffix) != dict.end()) {
//                 return memo[word] = true;
//             }

//             // Case 2: prefix is a word AND suffix can be formed recursively
//             if (dict.find(prefix) != dict.end() && isConcatenated(suffix, dict)) {
//                 return memo[word] = true;
//             }
//         }
...
//             // Important: remove the word itself temporarily to avoid self-match
//             dict.erase(word);
...
//             // Add the word back for future checks
//             dict.insert(word);
//         }
